...
class RobotDef(object):
    ...
    def _gen_symbols(self):
        ...
        self.q = sympy.Matrix([[_new_sym(r'q[' + str(i) + r']')]
                              for i in range(self.dof)])
        self.dq = sympy.Matrix([[_new_sym(r'dq[' + str(i) + r']')]
                               for i in range(self.dof)])
        self.ddq = sympy.Matrix([[_new_sym(r'ddq[' + str(i) + r']')] 
                                 for i in range(self.dof)])
        ...
        for i in range(dof):
            ...
            L_funcof_I[i] = I[i] + m[i] * _skew(r[i]).T * _skew(r[i])
            I_funcof_L[i] = L[i] - m[i] * _skew(r[i]).T * _skew(r[i])
